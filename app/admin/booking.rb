ActiveAdmin.register Booking do
  permit_params :startdate, :starttime, :user_id,
                :instructor_id, :body, :_destroy
  config.sort_order = 'startdate_asc'
  config.clear_action_items!
  filter :instructor, label: 'Inštruktor'
  filter :user_id, label: 'Užívateľ', as: :select,
         collection: User.all.map(&:email)
  ...
  index title: 'Rezervácie' do
    ...
    actions defaults: false do |booking|
      links = link_to 'Upraviť', edit_admin_booking_path(booking),
                                 class: 'member_link'
      links += link_to 'Odstrániť', admin_booking_path(booking),
                       method: :delete,
  ...
  form title: 'Rezervácia' do |f|
    f.inputs 'Údaje o rezervácii' do
      ...
      f.input :starttime, label: 'Termín', start_hour: 9,
                          end_hour: 15, minute_step: 60

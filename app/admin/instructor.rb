ActiveAdmin.register Instructor do
  ...
  index title: 'Inštruktori' do
    ...
    actions defaults: false do |instructor|
      links = link_to 'Rezervácie', admin_instructor_bookings_path(instructor),
                      class: 'member_link'
      links += link_to 'Odstrániť', admin_instructor_path(instructor),
                       method: :delete, data: { confirm: 'Ste si istí?' },
                       class: 'member_link'
      links
    end
  end
